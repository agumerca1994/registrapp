"""
Importador puntual de resúmenes de tarjeta Naranja X para RegistrApp.
Hermano de import_bbva_statements.py — mismo flujo (extract -> revisión manual
del JSON -> import --dry-run -> import --commit), parser distinto por el
formato propio de Naranja X. No comparte código con el de BBVA a propósito
(cada banco es un script independiente, ver memoria del proyecto).
...
Ejemplos:
    python -m scripts.import_naranjax_statements extract ../context/resumen-tarjeta-naranja*.pdf
    python -m scripts.import_naranjax_statements cards --email tu-email@ejemplo.com
    python -m scripts.import_naranjax_statements categories --email tu-email@ejemplo.com
    python -m scripts.import_naranjax_statements import scripts/_staging_naranjax/file.json --email tu-email@ejemplo.com --dry-run
"""
...
def cmd_extract(args: argparse.Namespace) -> None:
    ...
    for pdf_path in args.pdfs:
        ...
        for stmt in data["statements"]:
            ...
            print(f"  [{stmt['cardholder']}] Naranja X: {n_items} ítems ({n_installment} en cuotas)")
        print(f"  -> {out_path}")
        print()
...
async def _resolve_card(session, data, tenant_id, user_id):
    ...
    if card_info.get("create_new"):
        ...
        card = CreditCard(
            ...
            bank=card_info.get("bank", "Naranja X"),
    ...
    for idx, item_json in enumerate(stmt_data["items"]):
        ...
        try:
            item, n_future = await _import_item(
                ...
                propagate_future=stmt_data.get("is_latest_statement", False) or item_json.get("zeta_plan", False),
            )
            n_imported += 1
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p_extract = sub.add_parser("extract", help="Extrae datos de PDFs de resumen Naranja X a JSON")
    p_extract.add_argument("pdfs", nargs="+", type=Path)
    p_extract.add_argument("--out-dir", type=Path, default=Path(__file__).parent / "_staging_naranjax")
