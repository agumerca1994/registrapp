"""
Importador puntual de resúmenes de tarjeta BBVA Mastercard (Black) para RegistrApp.
Hermano de import_bbva_statements.py (Visa Signature) — mismo banco, mismo
formato de PDF exacto (verificado: parse_header/parse_statement de aquel
script corren sin cambios sobre estos PDFs), pero es una cuenta y tarjeta
física distinta ("Mastercard Black cuenta 1265842787" vs la Visa Signature),
así que es un script propio a partir de una copia — no se comparte código ni
se toca el original (ver memoria del proyecto).
...
Ejemplos:
    python -m scripts.import_bbva_mastercard_statements extract ../context/file*.pdf
    python -m scripts.import_bbva_mastercard_statements cards --email tu-email@ejemplo.com
    python -m scripts.import_bbva_mastercard_statements categories --email tu-email@ejemplo.com
    python -m scripts.import_bbva_mastercard_statements import scripts/_staging_bbva_mastercard/file.json --email tu-email@ejemplo.com --dry-run
    python -m scripts.import_bbva_mastercard_statements import scripts/_staging_bbva_mastercard/file.json --email tu-email@ejemplo.com --commit --force-import 0:3,1:1
"""
...
# A diferencia de la cuenta Visa Signature (que consolida 2 titulares), esta
# cuenta Mastercard Black solo tiene un titular — pero el nombre viene escrito
# distinto en este PDF ("Miguel Agus Mercado", no "Miguel A Mercado"), así que
# necesita su propia entrada de todos modos.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p_extract.add_argument("--out-dir", type=Path, default=Path(__file__).parent / "_staging_bbva_mastercard")
