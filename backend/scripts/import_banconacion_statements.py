"""
Importador puntual de resúmenes de tarjeta Visa Banco Nación para RegistrApp.
Hermano de import_bbva_statements.py / import_naranjax_statements.py — mismo
flujo (extract -> revisión manual del JSON -> import --dry-run -> import
--commit), parser distinto por el formato propio de Banco Nación. No comparte
código con los otros bancos a propósito (ver memoria del proyecto).
...
Ejemplos:
    python -m scripts.import_banconacion_statements extract ../context/Resumen_*.pdf
    python -m scripts.import_banconacion_statements cards --email tu-email@ejemplo.com
    python -m scripts.import_banconacion_statements categories --email tu-email@ejemplo.com
    python -m scripts.import_banconacion_statements import scripts/_staging_banconacion/file.json --email tu-email@ejemplo.com --dry-run
"""
    ...
    for pdf_path in args.pdfs:
        ...
        for stmt in data["statements"]:
            ...
            print(f"  [{stmt['cardholder']}] {card['hint_label']} ...{card['hint_last_4']}: "
                  f"{n_items} ítems ({n_installment} en cuotas)")
        print(f"  -> {out_path}")
        print()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p_extract = sub.add_parser("extract", help="Extrae datos de PDFs de resumen Banco Nación a JSON")
    p_extract.add_argument("pdfs", nargs="+", type=Path)
    p_extract.add_argument("--out-dir", type=Path, default=Path(__file__).parent / "_staging_banconacion")
