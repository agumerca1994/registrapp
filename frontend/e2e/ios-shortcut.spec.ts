 ...
 * la URL. (La app ya no interpreta ese texto: el lector se eliminó en 2026-09.)
 */
...
test("el Atajo aterriza en /registrar y avisa que se carga a mano", async ({ page }) => {
  // Un Atajo ya instalado sigue mandando el texto de la captura en la URL. El
  // lector que lo interpretaba se eliminó, así que lo que tiene que pasar es que
  // la pantalla abra igual y diga qué hacer, no que se rompa.
  ...
  await expect(page.getByText(/Llegaste desde un comprobante/)).toBeVisible();
