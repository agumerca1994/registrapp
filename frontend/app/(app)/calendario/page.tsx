...
function DayPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      <Card className="rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-5 space-y-4 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-foreground capitalize">{format(date, "EEEE d 'de' MMMM", { locale: es })}</h3>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground p-1"><X className="w-5 h-5" /></button>
        </div>

        {events.length === 0 ? (
          <p className="text-sm text-muted-foreground/70">Sin eventos este día.</p>
        ) : (
            ...
            {events.map((ev, i) => (
              <div key={i} className={cn(
                ...
                {ev.kind === "reminder" && ev.reminder && (
                  <button onClick={() => onDeleteReminder(ev.reminder!.id)} className="text-primary/50 hover:text-destructive shrink-0">
