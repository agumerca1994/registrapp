...
Current flags: `NEXT_PUBLIC_FEATURE_IOS_SHORTCUT` — the iOS Shortcut section in Settings (downloads + automation instructions), **paused**. Off only hides the offer: `/registrar?source=shortcut` keeps working, so an already-installed shortcut doesn't break.
