...
The suite runs in three Playwright projects, in this order: `chromium` (functional specs), `flows` (`new-expense.spec.ts`, the specs that create cards, shared expenses and entries through the API and delete them in `finally`) and `visual`, which **depends on both and runs last**. With everything in one project and `fullyParallel`, a screenshot could catch a card or expense from a flow half-way through. Two traps: a spec that creates data belongs in `flows`, not `chromium`; and don't make `flows` depend on `chromium` — Playwright skips dependents when a dependency fails, and the visual project has baselines that drift with the date. `visual` sets `snapshotPathTemplate` with a fixed `-chromium` suffix so it keeps reading the existing `*-chromium-darwin.png` baselines.
...
**Feature flags live in `frontend/lib/features.ts`, and only there.** Each one is a `NEXT_PUBLIC_FEATURE_*` variable read as `=== "true"`, so **a flag is off unless explicitly turned on** — a paused feature must not come back because someone forgot to set a variable. Three things a new flag needs, and missing any of them leaves it silently off in production: the literal `process.env.NEXT_PUBLIC_FEATURE_X` in `features.ts` (Next only inlines the literal expression; `process.env[name]` is `undefined` in the browser), `ARG` + `ENV` in `frontend/Dockerfile.prod`, and the entry under both `args` and `environment` in `docker-compose.prod.yml` with a `:-false` default. Turning one on is setting the variable to `true` in Easypanel **and redeploying**. E2E tests that depend on a flag read the same variable and `test.skip` accordingly; `playwright.config.ts` forwards it to the dev server it starts (but not to one already running, because of `reuseExistingServer`).

Current flags: `NEXT_PUBLIC_FEATURE_IOS_SHORTCUT` — the iOS Shortcut section in Settings (downloads + automation instructions), **paused**. Off only hides the offer: `/registrar?source=shortcut` keeps working, so an already-installed shortcut doesn't break. Note it no longer fills the form: **the receipt reader (`/receipts/parse`, "Pegar comprobante" / "Subir PDF") was removed in 2026-09**, so the Android share target and this shortcut both land on `/registrar` with a "complete it by hand" notice — the page still consumes the share target's IndexedDB payload so it doesn't linger.
...
**Los canales de aviso son dos, y ninguno es el mail.** El **push del sistema** es la base: se intenta siempre para cualquier participante con cuenta, no se elige ni se apaga, y es el único que no depende de tener un número vinculado ni de un proveedor externo. **WhatsApp es secundario y opcional**, gobernado por `users.whatsapp_notifications` — **un solo permiso que vale en las dos direcciones**: quien comparte autoriza que se avise por WhatsApp en su nombre, y quien recibe autoriza que le llegue. Un mensaje sale sólo si las dos personas lo permitieron y el destinatario además tiene número vinculado.

Consecuencia que el interruptor tiene que declarar en pantalla: **con el permiso apagado, invitar a alguien que no tiene la app no le manda nada**. WhatsApp es su único canal — no tiene dónde recibir un push ni preferencias que consultar. Decirlo al lado del interruptor y no cuando ya pasó.

**Mail no existe como canal y se sacó de la app**, incluido el `mailto:` que tenía "Invitar amigo" en Configuración: ofrecer un botón de Email ahí era la única puerta a un canal que no existe en ningún otro lado. Invitar por una dirección de mail sigue sirviendo para **identificar** a alguien —el día que se registre con esa dirección, el gasto lo está esperando vía la cláusula 3 de `_load_q`— pero **no se le envía nada**, y el sub-formulario lo dice con todas las letras en vez de simularlo. Quien comparte copia el link de invitación desde `/shared` y lo pasa por donde quiera. Si alguna vez se suma un proveedor de mail, lo que cambia es ese aviso y el envío; nada más.

A quien **no** tiene cuenta y se invita por teléfono sí le sale un WhatsApp: no tiene app donde recibir el push ni preferencias que consultar, y ése es justamente el mecanismo que trae usuarios nuevos.

**Elegir participantes es `components/ParticipantPicker.tsx`, uno solo para las dos pantallas.** Antes había dos selectores en línea —el de `/shared` y el de `ShareItemModal` en tarjetas— que habían divergido: el de tarjetas era un campo de texto suelto "WhatsApp o email" sin prefijo de país, que producía JIDs inválidos para un número recién tipeado. Va en `components/` y no en `components/ui/`: esa carpeta es el kit de primitivas y esto consulta `/directory` y `/contacts`, o sea una feature de dominio, como `WhatsAppVerifyForm`.

**Se eliminó el selector "Del hogar / Externo".** Ese toggle obligaba a clasificar a una persona antes de nombrarla, y es lo que hacía incómodo compartirle a alguien de afuera del hogar. En su lugar hay un solo buscador: se escribe nombre, alias, mail o teléfono, y las secciones se arman solas (*De tu hogar* · *Frecuentes/Tus contactos* · *En RegistrApp* · *Otras opciones*).
...
`applyPick` traduce lo que devuelve el selector a la fila que cada formulario maneja; **los montos NO se unificaron** (`/shared` tiene `redistAuto` y flags por fila, el de tarjetas redondea el resto en la última) porque esa divergencia es legítima. Ojo con el teléfono: el selector entrega el contacto completo con prefijo, así que `applyPick` guarda los dígitos enteros y deja `invite_phone_prefix` vacío — anteponerle otro prefijo duplicaría el código de país.
...
**Creating an item without an open statement: `POST /credit-cards/{card_id}/items`.** Body is `CreditCardItemCreate` plus `year`/`month` and an optional `share` (`ShareCreditCardItemBody`). The statement is found or created by period with `_find_or_create_statement` — the app does not know any card's closing day, so the period is always the user's choice. With `share`, the item is created and shared **in one transaction**: it validates the split before writing anything (at least two people, sum equal to the per-cuota amount) and commits once. "Recurrente" is rejected here, because nothing actually repeats it (only `finalize_statement` copies it forward, and the frontend never calls `/finalize`). It's what the unified expense form uses; `CreditCardItemOut` now carries `statement_id` for its "Ver resumen" link.

It shares its body with the per-statement routes through two helpers that **only flush, never commit**: `_create_item_in_statement` (item, mirrored entry, future cuotas) and `_share_item_in_tx` (one `SharedExpense` per cuota, splits, agenda, creator's entry rewritten to their share). `create_item`, `share_item` and the new route are all "load → helper → commit → notify → respond". **Notifications go after the commit, always.** `share_item` used to call `notify_share` inside its loop, and the push path drops dead device tokens with its own `db.commit()` (`push._drop_tokens`): one dead device committed half a share, and the notice could go out for a share that then failed. `_share_item_in_tx` takes `card` explicitly instead of reading `item.statement.card`, because on an item created in the same session that relationship isn't loaded and lazy-loading it raises `MissingGreenlet`.
...
**`POST /shared-expenses` takes `currency`** (`"ARS"` by default, so `/shared`, which never sends it, behaves as before) and resolves the category like `POST /expenses/entries`: optional in USD (falls back to "Consumo en dólares"), required in ARS, and **checked with `assert_owns_category`**. It used to write the client's `category_id` unchecked, and since the creator's entry embeds its category, that leaked other households' category names. The currency also reaches the WhatsApp texts (`whatsapp._money`: `U$D` instead of a hard-coded `$`, which was already wrong for USD card shares) and the `/internal` backfill and resend.

**Splitting amounts has three implementations, on purpose for now.** The unified expense form uses `frontend/lib/split.ts` (cents, remainder to the last row, exact sum). `/shared` and `ShareItemModal` keep their own logic because the user chose not to touch those forms, and both have known defects: `/shared`'s `parseAmt` reads "15.000" as 15 and doesn't close the equal-split remainder (100 / 7 doesn't add up), and `ShareItemModal` accepts ±0,02 while the backend allows ±0,01. When either form is touched, move it to `lib/split.ts`.
...
**Resolver un participante vive en `services/participants.py`, no en el router.** `routers/credit_cards.py` tenía una copia de esa lógica y derivó: le faltaba la rama del externo sin cuenta —creaba splits `pending` con `user_id=NULL` y sin token, que `_my_split` nunca matchea y por lo tanto nadie puede aceptar ni rechazar jamás— y le pasaba un mail al enviador de WhatsApp como si fuera un teléfono. Los dos bugs vienen de reimplementar, no de equivocarse, así que **lo que varía entre routers es un parámetro (`mint_token`), no una segunda implementación**: `resolve_participant()` devuelve un `ResolvedParticipant` cuyo `.status` ya sabe que un externo sin cuenta nace `accepted`. La migración `f2a3b4c5d6e7` reparó las filas que la copia dejó. Los enviadores de WhatsApp se mudaron por el mismo motivo a `services/whatsapp.py` (los usan `auth.py` para el OTP y `reminders.py` para los recordatorios, que no tienen nada que ver con compartir), y avisar que se compartió algo es `services/notify_shared.py` — antes compartir la cuota de una tarjeta mandaba **sólo WhatsApp**, o sea que no le llegaba a nadie sin número vinculado. Todos estos helpers se re-exportan con sus nombres privados desde `routers/shared_expenses.py` porque siete módulos los importan de ahí.
...
Un gasto compartido se anunciaba sólo dentro de `/shared`: quien no entraba ahí no se enteraba. Hay tres avisos, y cada uno cubre lo que el anterior no puede.

**`GET /shared-expenses/pending`** devuelve los gastos esperando la decisión de quien pregunta, y alimenta tanto el diálogo del primer ingreso como el puntito de la navegación. Devuelve las filas y no un conteo a propósito: el diálogo necesita los montos y el puntito el número, y dos endpoints contestando "cuántos hay pendientes" es el par que se desincroniza. `_pending_q()` repite las reglas de pertenencia de `_load_q()` — split propio, o invitación sin reclamar dirigida a su mail/teléfono — con una omisión deliberada: **no incluye `tenant_id`**. `_load_q` sí lo ORea, porque en la lista todos ven todo lo del hogar; acá sería el puntito prendido por una decisión que no es tuya.

**El diálogo** (`PendingSharedDialog`, montado en el layout de `(app)`) pagina de a uno. No tiene "omitir": no aceptar ni rechazar es exactamente lo mismo que cerrar, y dos acciones para un mismo resultado sólo obligan a elegir entre ellas. **Se aparta mientras corre la guía de producto**, y ese chequeo tiene que ser `useTourRunning()` — el estado vivo que publica `ProductTour` — y no la clave `tour_seen_*`: un tour con `requireDesktop` sale sin marcarse como visto, así que en un teléfono esa clave nunca se escribe y preguntarle a localStorage daba "hay guía pendiente" para siempre. Esa fue exactamente la causa de que el aviso no apareciera nunca en iOS mientras el puntito sí, porque el puntito no pasa por esa compuerta.

**El puntito** sale de `PendingSharedProvider`, y todo componente que lo pinte tiene que llamar a `usePendingShared()` — misma trampa que documenta `PrivacyProvider`: las pantallas llegan como `children` y React no las re-renderiza cuando cambia el estado del provider. Va en tres lugares (sidebar de escritorio, ícono de "Más" en la tab bar, ítem dentro de la hoja) y es ámbar, que es lo que la app ya usa para "pendiente"; no rojo, que es error.

**Push (FCM)** — `services/push.py` + `routers/notifications.py` + `models/device_token.py`. No agrega proveedor: manda con la misma credencial de `firebase-admin` que ya verifica los tokens de Auth. Cuatro cosas que no hay que deshacer:
- **`fcm_options.link` exige una URL absoluta y HTTPS.** Pasarle `/shared` hace que `firebase-admin` levante `ValueError` al *armar* el mensaje, antes de tocar la red — o sea que rompe igual con FCM perfectamente configurado. Se construye desde `FRONTEND_URL` y se omite si no es https (dev local); el service worker igual navega con `data.url`.
- **`send_each_for_multicast` es sincrónico**: va por `asyncio.to_thread` o traba el event loop de todo el backend mientras habla con Google.
- **Un fallo mandando nunca tumba el request.** Compartir tiene que funcionar con FCM caído, igual que ya pasa con WhatsApp.
- **Los tokens muertos se borran solos** cuando FCM contesta que ya no existen; si no, la tabla se llena de destinos inválidos.

`device_tokens.token` es único **global**, no por usuario: dos cuentas en el mismo navegador comparten el token que da FCM y el último que lo registra es su dueño — repetirlo mandaría el aviso de una persona a la pantalla de la otra. El frontend re-registra en cada arranque (`syncPushToken`), porque FCM rota el token y deja de entregar al viejo sin avisar.

**En iOS el permiso no alcanza: Safari sólo entrega push si la app está instalada en la pantalla de inicio.** Sin instalar no existe `PushManager` y el permiso no se puede ni pedir. Por eso `pushState()` no devuelve un booleano sino el motivo (`unsupported` / `ios-needs-install` / `prompt` / `denied` / `granted`) y la sección de Configuración muestra un texto distinto para cada uno — un botón que no hace nada es lo peor que puede haber ahí. Para `denied` **no existe URL a la que un botón pueda llevar** en ningún navegador: lo único honesto es explicar dónde queda en los ajustes del sistema. Y ese texto **no puede afirmar que el usuario rechazó el permiso**: `Notification.permission` devuelve `"denied"` también cuando el navegador tiene las notificaciones apagadas a nivel general, sin que nadie haya tocado nada en este sitio — decirle "lo rechazaste una vez" lo manda a buscar un permiso por sitio que no existe. El estado `insecure` (`!window.isSecureContext`) se chequea **antes que el permiso** por lo mismo: servido por una IP de la LAN en vez de `localhost`, el navegador reporta `denied` sin que haya nada que habilitar.

**Permiso concedido no es lo mismo que dispositivo registrado, y confundirlos deja el problema sin dónde mirar.** Si falta la VAPID key, `enablePush()` consigue el permiso, nunca pide token y devuelve `granted` igual: la pantalla decía "Activadas en este dispositivo" y no llegaba nada, sin un solo error en los logs — porque no falla nada, sencillamente no hay a dónde mandar. `isDeviceRegistered()` (hay token guardado) es la segunda pregunta, y la sección muestra un aviso ámbar con el motivo concreto (`pushConfigProblem()`) y un botón de reintentar cuando hay permiso sin registro. `GET /internal/device-tokens?email=` contesta lo mismo desde el lado del servidor, que es lo único que sirve cuando el que reporta está en otro dispositivo.

`NEXT_PUBLIC_FIREBASE_VAPID_KEY` sale de la consola (Configuración del proyecto → Cloud Messaging → Certificados push web) y, como todo `NEXT_PUBLIC_*`, **se hornea en tiempo de build**: hay que declararla en `Dockerfile.prod` (ARG + ENV) *y* en los `args` de `docker-compose.prod.yml`, no sólo en `environment`, o se setea en Easypanel y no tiene ningún efecto. El proyecto del emulador (`demo-registrapp`) no puede mandar push: en dev local todo se registra igual y el envío falla logueado.

### Perfil: nombre, apellido, alias y hogar
`users.first_name` y `users.last_name` son los campos que se piden y se editan; **`display_name` se sigue guardando, derivado de los dos** (`compose_display_name` en `routers/auth.py`). Guardarlo derivado y no componerlo al leer es deliberado: es la clave por la que busca el directorio, la que agrupa `personKey()` y la que **quedó escrita en cada split ya creado** — recalcularla en cada lectura haría que cambiarse el nombre reescriba la historia de gastos ya cerrados. La migración `d6e7f8a9b0c1` hace el backfill partiendo por el primer espacio (primer token = nombre, resto = apellido, que es lo correcto para "María del Carmen García") y **no toca `display_name`**: nadie tiene que ver que su nombre cambió por una migración.

El alta (`/onboarding`) pide los cuatro: nombre, apellido, alias y nombre del hogar. **El alias se pre-llena solo con `suggestAlias()`** (`frontend/lib/alias.ts`, compartido con Configuración para que no den sugerencias distintas para la misma persona) y deja de seguir al nombre en cuanto el usuario lo toca — un campo que se reescribe mientras lo estás mirando es peor que una sugerencia mediocre.

En Configuración, "Tus datos" **muestra** —nombre y alias como chips, el alias con su botón de copiar al portapapeles— y el menú de **⋮** (el mismo `DropdownMenu` de tarjetas y resúmenes, no un enlace propio de esta pantalla) abre un modal que edita todo. Es modal y no un formulario siempre abierto porque esa pantalla es casi toda formularios, y estos campos se usan donde un cambio se nota. El nombre del hogar va en el mismo modal pero por otro endpoint (`PATCH /auth/tenant`, **sólo admin**): es del hogar, no tuyo, y lo ve todo el mundo. `UserOut` expone `tenant_name` como `@property` igual que `tenant_code` — misma trampa, necesita `selectinload(User.tenant)` en todo endpoint que lo devuelva.

### Alias y directorio de usuarios
Para compartirle un gasto a alguien hacía falta saber su mail o su teléfono. `users.alias` es el equivalente al alias de una transferencia bancaria: algo corto que se dice en voz alta o se pega en un chat. **Se guarda siempre en minúscula** (`services/user_directory.validate_alias`), así el único y la búsqueda son comparaciones directas — eso esquiva la clase de bug de mayúsculas que ya costó cara acá (`_find_user_by_email` normalizaba y `_link_pending_splits` no, y los invitados con mayúsculas en el mail no se vinculaban nunca). **No se autogenera**: un alias asignado solo es un alias que nadie sabe que tiene, y entonces nunca se dice ni se pega, que era todo el punto; el formulario pre-llena una sugerencia derivada del nombre con `foldText()` y se acepta con un toque. Un alias tomado es **409**, no 400 — para el frontend "inválido" y "ya está en uso" son mensajes distintos y sólo el segundo se arregla probando otro.

`/directory` tiene **dos endpoints deliberadamente asimétricos**. `GET /directory/lookup?q=` resuelve un dato exacto (alias, mail o teléfono) y **no revela nada nuevo**: `POST /shared-expenses` ya resolvía cualquier mail o teléfono contra la tabla global desde siempre; esto lo hace explícito y por lo tanto limitable. Devuelve **200 con `found:false`, nunca 404** — una diferencia de código de estado *es* el oráculo que se quiere evitar. `GET /directory/search?q=` busca por nombre, que sí es superficie nueva: mira `discoverable`, exige 3 caracteres y **no tiene `offset`**, porque un offset convierte una búsqueda en un volcado de la tabla de usuarios.

**`discoverable` gobierna sólo la búsqueda por nombre, nunca la exacta.** Si apagara también la exacta, quien lo desactiva dejaría de recibir en silencio los gastos que le compartan por mail o teléfono — el fallo más caro de este dominio, porque no da error, simplemente no llega nada. Arranca en `true` para todos: con `false` por defecto la búsqueda nace vacía, los primeros usuarios concluyen que está rota, y de esa primera impresión no se vuelve.

`DirectoryUserOut` es un schema **propio y no `UserOut`** porque ese incluye `tenant_code`, el código para unirse a un hogar: devolverlo ahí le daría a cualquiera una forma de meterse en el hogar de cualquier usuario que pueda encontrar por nombre. Tampoco van mail ni teléfono **ni siquiera enmascarados** — un `j•••@gmail.com` sigue siendo un oráculo de confirmación, sólo que disfrazado de privacidad; desambiguar entre homónimos es el trabajo del alias. El límite de uso aplica **dos baldes**, por usuario y por IP (`rate_limit.enforce_key`): en la Argentina los usuarios móviles comparten IP por el NAT del operador, así que limitar sólo por IP castiga a gente que no hizo nada y no frena a quien rota de red; el de IP es el que frena rotar cuentas desde un mismo host.

### Contacts (household agenda)
**`SharedContact` reemplaza a `TenantContact`** (migración `b4c5d6e7f8a9`; la tabla vieja sobrevive una release porque `GET/DELETE /internal/tenant-contacts` son herramientas de ops y romperlas en medio de un incidente es peor que la duplicación). La vieja no podía crecer: `contact_phone` era NOT NULL *y* mitad de la clave única, así que un contacto sólo-mail no tenía dónde ir. Sigue siendo **por hogar** — como recordatorios, tarjetas y categorías —; lo "de usuario a usuario" va en `contact_user_id`, no en la propiedad de la tabla.

La clave única es `(tenant_id, person_key)`, y **`person_key` es el gemelo backend de `personKey()`** del frontend, misma precedencia `u:` → `p:` → `e:` → `n:`. Se **guarda** en vez de calcularse al leer, para que la restricción exprese la regla de identidad real de la app. Que `u:` gane es lo que evita que una persona se convierta en dos filas el día que se registra: `link_contact_to_user()` (llamada desde `_accept_split` y `_link_pending_splits`) promueve la fila y fusiona si ya había una. `use_count`/`last_used_at` se guardan en vez de derivarse contando splits — eso sería un join sobre todos los gastos compartidos cada vez que se abre el selector.

**El alta corre ahora también para los contactos por mail.** Antes sólo la rama de teléfono guardaba agenda, y ése era el único motivo de que "Elegir de la agenda" nunca mostrara un contacto de mail. Y no se guarda a sí mismo: `if not r.is_creator`.
...
frontend/app/
  ...
  (app)/            # Protected layout (sidebar + auth guard)
    ...
    expenses/       # Expense entries; search + filters, "+" FAB; credit card entries show
                    #   badge + "Ver en resumen" only. Creating and editing go through
                    #   components/expense/ExpenseFormModal (see "Unified expense form")
...
**Unified expense form (`components/expense/`).** The `+` on `/expenses` and on the dashboard (via `/expenses?nuevo=1`) opens `ExpenseFormModal`, one form for every kind of expense: simple, credit card, shared, or card + shared. **One form and not a type picker**, because "card" and "shared" aren't exclusive — a three-option menu can't represent a shared card expense without a fourth option, and it adds a tap to the most common case. The base fields are the old `/expenses` form (`BaseFields`); below them, "Pago" (efectivo o débito | tarjeta) and "¿Lo compartís?" (no | sí) start collapsed, so a simple expense is loaded exactly as before and makes no extra request (cards are fetched the first time "Tarjeta" is chosen). `submit.ts` holds the draft, the cross-field rules (`validateDraft`) and the four-way request matrix (`buildRequest`): simple → `POST /expenses/entries`; card → `POST /credit-cards/{id}/items`; shared → `POST /shared-expenses`; card + shared → the card route with `share`, atomic. `/shared` and the card statement page still have their own forms — the user chose to leave them.

Rules the form encodes: with installments the amount becomes "Monto por cuota" and **what gets split is the per-cuota amount** (the backend shares each cuota with the same amounts), and the screen says so; USD with a card is single-payment only and "En cuotas" shows disabled with the reason instead of disappearing; the statement month follows the purchase date until the user touches it, with a note that the app doesn't compute the card's closing; with no cards there's an empty state and Guardar stays disabled. An error keeps the modal open with the draft intact. Editing uses the same modal without the two questions (turning a saved entry into a card or shared expense isn't supported), preloads the amount in local format and preserves `notes`, which has no field. `ParticipantPicker` and `NewCategoryModal` are rendered **outside the `<form>`**: the picker's search box doesn't stop Enter (it would submit the expense mid-pick), and the category sub-modal's backdrop click would bubble to the form's backdrop and close everything. `SegmentedToggle` (in `components/ui/form.tsx`) is the generic segmented control; `CurrencyToggle` wraps it with identical DOM.
...
- **Backend**: Use `normalize_phone(value)` from `services/participants.py` (re-exported as `_normalize_phone` from `routers/shared_expenses.py` for the callers that predate the move) to normalize before storing/comparing. It always inserts the mandatory `9` after the `54` country code for Argentine numbers (even for a bare 10-digit local input) — don't reintroduce a code path that only *preserves* a `9` the caller already included, that silently produces invalid JIDs for the common case of a freshly-typed number.
- **Evolution API**: Expects full international format like `+54934567890`.
- **El webhook entrante fue el caso real de esta regla, y costó caro.** `POST /webhook/whatsapp` buscaba con `User.whatsapp_phone == phone`, pero el JID de Evolution llega **sin `+`** (`5493834373344@s.whatsapp.net`) y `verify_whatsapp` guarda el número normalizado **con `+`**. La comparación nunca matcheaba: el bot contestaba "número no vinculado" a gente que sí lo tenía vinculado, y que además recibía los avisos de gastos compartidos —esos sí usan la búsqueda tolerante— así que el síntoma parecía imposible. No dejó ni un log: no falla nada, simplemente no encuentra.
...
- **Sending is a two-step resolve-then-send, not a direct `sendText` call**: Evolution API's `/message/sendText/{instance}` endpoint has its own internal number-existence check that is stricter (and inconsistent) with its dedicated `/chat/whatsappNumbers/{instance}` lookup endpoint — a number the lookup confirms `exists: true` can still get rejected by `sendText` with `exists: false` for the exact same string. `resolve_whatsapp_jid()` in `services/whatsapp.py` calls the lookup endpoint first and sends to whatever canonical number it returns (falling back to the locally-normalized number only if the lookup itself fails); both `_send_wa_msg` and `/auth/me/link-whatsapp` go through it. Diagnose future "WhatsApp doesn't arrive" reports with `GET /internal/whatsapp-check?phone=` before assuming it's a formatting bug.

**Un 2xx de Evolution significa "lo acepté", no "lo entregué".** Con la instancia desconectada de WhatsApp responde 200 igual y el mensaje no sale nunca: el usuario ve "código enviado" y espera para siempre. Eso produjo un reporte de "no puedo vincular mi número" **sin un solo rastro en los logs** — no fallaba nada, simplemente no llegaba, y `recent_errors`/`search_logs` daban cero. Por eso `link_whatsapp` ahora registra el resultado: `WARNING` cuando el número no resolvió en Evolution (el código se manda a ciegas) y el cuerpo de la respuesta cuando Evolution rechaza. Antes de sospechar del código, verificar la instancia: **cero errores en los logs no descarta el problema, lo confirma como de entrega.**
