...
All of it is wired as VS Code tasks (Cmd+Shift+P → "Tasks: Run Task") with keyboard shortcuts: `Cmd+Alt+G` commit+push, `Cmd+Alt+D` deploy, `Cmd+Alt+C` the deploy preflight alone, `Cmd+Shift+B` the frontend build. Two deliberate details: the build is the *default* build task, so the easiest shortcut goes to the action that can't break production; and every task runs in the integrated terminal and confirms there, so a mis-hit shortcut never commits or deploys on its own. The task file is duplicated at `/Users/agustin/Proyectos/.vscode/tasks.json` (outside the repo, not committed) because VS Code only reads the `.vscode/` of the folder that is actually open, and that machine has the parent folder open — the labels are identical in both, so the shortcuts work either way.
